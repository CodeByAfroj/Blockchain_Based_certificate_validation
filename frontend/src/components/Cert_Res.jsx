...
import { Calendar, User, GraduationCap, Hash, Download, Share2 } from "lucide-react";
...
export default function Cert_Res() {
  ...
  const handleVerify = async () => {
    ...
    try {
      const params = /^\d+$/.test(query) ? { id: query } : { name: query };
                ...
                <div className="lg:col-span-2 space-y-4">
                  ...
                  {verifyResult.type === "name" && Array.isArray(verifyResult.certificates) && (
                    <div className="space-y-4 bg-gray-800 p-4 rounded-md">
                      <div className="flex items-center gap-2">
                        <User className="w-5 h-5 text-teal-400" />
                        <span className="font-semibold text-gray-200">Certificates</span>
                      </div>
                      {verifyResult.certificates.map((c) => (
                        <div key={c.certificateID} className="border border-gray-700 p-2 rounded-md">
                          <p><strong>{c.course}</strong></p>
                          <p>Date Issued: {new Date(c.dateIssued).toLocaleDateString()}</p>
                          <p>Certificate ID: {c.certificateID}</p>
                          <div className="mt-2">
                            <QRCodeSVG value={c.certificateID} size={128} />
                          </div>
                          <div className="flex gap-2 mt-2">
                            <button size="sm" onClick={() => handleDownloadReport(c)}>
                              <Download className="w-4 h-4 mr-1" /> Download
                            </button>
                            <button size="sm" onClick={() => handleShare(c)}>
                ...
                {verifyResult.type === "id" && verifyResult.certificate && (
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col gap-2">
                      <button onClick={() => handleDownloadReport(verifyResult.certificate)} variant="outline">
                        <Download className="mr-2 w-4 h-4" /> Download Report
                      </button>
                      <button onClick={() => handleShare(verifyResult.certificate)} variant="outline">
